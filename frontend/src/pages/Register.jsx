...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      login(res.user);
      toast.success(`Welcome to SpendSense, ${res.user.name.split(' ')[0]}!`);
      navigate('/dashboard');
    } catch (err) {
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              />
              {errors.name && (
                <p className="text-xs mt-1" style={{ color: 'var(--color-danger)' }}>{errors.name.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.confirmPassword && (
                <p className="text-xs mt-1" style={{ color: 'var(--color-danger)' }}>{errors.confirmPassword.message}</p>
              )}
            </div>
